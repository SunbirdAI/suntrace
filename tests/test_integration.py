...
class TestGeospatialAnalyzerIntegration:
    ...
    def test_full_workflow_building_analysis(self, analyzer_with_data, test_region):
        ...
        if building_count > 0:
            ...
            buffered_count = analyzer_with_data.count_buildings_within_region(buffered_region)
    ...
    def test_full_workflow_ndvi_analysis(self, analyzer_with_data, test_region):
        ...
        if (analyzer_with_data._joined_tiles_gdf.empty or 
            'ndvi_mean' not in analyzer_with_data._joined_tiles_gdf.columns):
    ...
    def test_cross_layer_consistency(self, analyzer_with_data, test_region):
        ...
        tile_count = analyzer_with_data.count_features_within_region(test_region, 'tiles')
    ...
    def test_large_region_analysis(self, analyzer_with_data):
        ...
        if len(bounds) == 4:
            ...
            building_count = analyzer_with_data.count_buildings_within_region(large_region)
            assert isinstance(building_count, int)
            assert building_count >= 0
    ...
    def test_data_integrity_checks(self, analyzer_with_data):
        ...
        if not analyzer_with_data._joined_tiles_gdf.empty:
            ...
            stat_cols = [col for col in analyzer_with_data._joined_tiles_gdf.columns 
