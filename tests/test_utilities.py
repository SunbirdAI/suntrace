...
class TestGeospatialAnalyzerUtilities:
    ...
    def test_ensure_crs_for_calculation_same_crs(self, mock_minimal_analyzer):
        ...
        result_geom, was_reprojected = (
            mock_minimal_analyzer._ensure_crs_for_calculation(test_geom, target_crs)
        )
        ...
        with patch("utils.GeospatialAnalyzer.gpd.read_file") as mock_read_file:
            with patch("pandas.read_csv") as mock_read_csv:
                ...
                mock_gdf = gpd.GeoDataFrame(
                    {"geometry": [Point(0, 0), Point(1, 1)], "test_col": [1, 2]},
                    crs="EPSG:4326",
                )
...
def test_crs_handling_parametrized(crs_input, expected_output):
    ...
    try:
        ...
        assert crs.to_string() == expected_output or crs.to_epsg() == int(
            expected_output.split(":")[1]
        )
    except Exception:
